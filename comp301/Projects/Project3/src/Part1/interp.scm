    ...
    (lambda (exp env)
      (cases expression exp
        ...
        (proc-nested-exp (var count name body)
                         (proc-val (nested-procedure var body env name count)))
        (call-nested-exp (rator rand count)
            (let ((proc (expval->proc 
                          (cases proc (expval->proc (value-of rator env))
                            (nested-procedure (bvar body saved-env name bcount) 
                                              (value-of (proc-nested-exp bvar bcount name body) (extend-env 'count (value-of count saved-env) saved-env)))
                            (else (value-of rator env)))))
                  (arg (value-of rand env)))
                        (begin 
                            ; (display (value-of count env))
                            ; (display "\n")
                            (apply-procedure proc arg))))
  ...
  (define apply-procedure
    (lambda (proc1 arg)
      (cases proc proc1
        ...
        (nested-procedure (bvar body saved-env name count)
          (begin
            (recursive-displayer name (expval->num (apply-env saved-env count)))
            (value-of body (extend-env-rec-nested name bvar body (extend-env bvar arg saved-env) count))
            ))
