(module tests mzscheme
  ...
  (define test-list
    '(
      ...
      ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
      (let-proc-letrec 
        "let g = proc(x) if zero?(x) then 0 else -((g -(x, 1)), -1)
            in letrec f(y) = if zero?(y) then 0 else -((g y), -(0, (f -(y,1))))
                in (f 3)"
            6)
      ...
      ;; ........fib --> 3
      (fact-letrec 
        "let mult = proc(x) proc(y) if zero?(x) then y else -(y, -(0, ((mult -(x, 1)) y)))
        in letrec fact(n) =
            if zero?(-(n,1))
                then 1
                else if zero?(n)
                  then 1
                  else ((mult -(n, 1)) (fact -(n, 1)))
                in (fact 5)"
        120)
      ;; Recursive Print:
      ;; fact --> 1
