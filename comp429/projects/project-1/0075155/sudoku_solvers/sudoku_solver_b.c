...
int solveSudoku(int row, int col, int matrix[MAX_SIZE][MAX_SIZE], int box_sz, int grid_sz, int depth) {
    ...
    if(matrix[row][col] != EMPTY) {
        return solveSudoku(row, col + 1, matrix, box_sz, grid_sz, depth + 1);
    } else {
        int num;
        for (num = 1; num <= box_sz; num++) {
            if (canBeFilled(matrix, row, col, num, box_sz, grid_sz)) {
                int local_matrix[MAX_SIZE][MAX_SIZE];
                memcpy(local_matrix, matrix, sizeof(int) * MAX_SIZE * MAX_SIZE);
                local_matrix[row][col] = num;

                if (depth < CUTOFF_DEPTH) {
                    #pragma omp task firstprivate(local_matrix)
                    {
                        solveSudoku(row, col + 1, local_matrix, box_sz, grid_sz, depth + 1);
                    }
                } else {
                    solveSudoku(row, col + 1, local_matrix, box_sz, grid_sz, depth + 1);
                }
