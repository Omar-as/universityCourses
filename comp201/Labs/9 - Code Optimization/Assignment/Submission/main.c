...
/* You need to optimize this function 
   ...
   Remove first nested loop. No need to initialize result. (Bonus Point)
   10-times loop unrolling for the inner loop of the second nested loop.
*/
void opt_matMul(int **first, int **second, int **result, int size) {
   ...
   for (i = 0; i < size; i++) {
      for (j = 0; j < size; j++) {
         for (k = 0; k < size - 9;) {
            result[i][j] += first[i][k] * second[k][j]; k++;
            result[i][j] += first[i][k] * second[k][j]; k++;
            result[i][j] += first[i][k] * second[k][j]; k++;
            result[i][j] += first[i][k] * second[k][j]; k++;
            result[i][j] += first[i][k] * second[k][j]; k++;
...
/* You need to optimize this function 
*/
void opt_foo1_2(int **result, int size){
	int a = 1, b = 5, c = 25, d = 7, i, j;
	int x = ((c % d) * a / b) % d;
...
int main (int argc, char *argv[])
{
  ...
  switch(atoi(argv[1])){
	...
	case 1:
		...
		// opt_foo1_2(opt_mul_res, atoi(argv[2]));
		opt_end   = clock();
